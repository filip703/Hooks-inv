...
function PullToRefresh({ onRefresh, color = '#D4AF37', bg = 'rgba(0,0,0,0.7)', children }) {
  ...
  const handleEnd = async () => {
    ...
    isPulling.current = false
    if (wasPulling && dist > 70 && !refreshing) {
      setRefreshing(true)
      try { await onRefresh?.() } catch (e) { console.warn('refresh', e) }
  ...
  return (
    <div onTouchStart={handleStart} onTouchMove={handleMove} onTouchEnd={handleEnd} onTouchCancel={handleEnd}>
      <div style={{ position: 'fixed', top: 'env(safe-area-inset-top, 0px)', left: 0, right: 0, height: 60, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', paddingBottom: 8, zIndex: 100, pointerEvents: 'none', transform: `translateY(${refreshing ? 0 : dist - 60}px)`, opacity: refreshing ? 1 : Math.min(dist / 70, 1), transition: (dist === 0 && !refreshing) ? 'transform 0.25s ease, opacity 0.25s ease' : 'none' }}>
        <div style={{ background: bg, backdropFilter: 'blur(10px)', borderRadius: 20, padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 8, border: `0.5px solid ${color}55` }}>
          <div style={{ width: 16, height: 16, border: `2px solid ${color}33`, borderTopColor: color, borderRadius: '50%', animation: refreshing ? 'p2r-spin 0.8s linear infinite' : 'none', transform: refreshing ? 'none' : `rotate(${dist * 4}deg)` }} />
          <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color, letterSpacing: 1 }}>{refreshing ? 'UPPDATERAR…' : dist > 70 ? 'SLÄPP FÖR UPPDATERING' : 'DRA NEDÅT'}</div>
        </div>
      </div>
      <style>{`@keyframes p2r-spin { to { transform: rotate(360deg) } }`}</style>
      {children}
    </div>
  )
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    else setAppMode('taby') // Täby OOM är standardläge — DIO nås via "byt läge" eller ?mode=dio
  }, [])
...
function TaByApp({ onSwitchMode, tabyOnly }) {
  ...
  const [tabySortBy, setTabySortBy] = useState('meritTotal') // 'meritTotal' | 'pi' | 'eventPoints' | 'h2hPct' | 'fullRounds'
  const [meritBreakdownPid, setMeritBreakdownPid] = useState(null) // player_id för öppen breakdown-modal
  ...
  return (
    ...
    <div style={{ minHeight: '100vh', background: '#0C1830', color: '#F0F4FF', paddingBottom: 80 }}>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 200, display: 'flex', flexDirection: 'column', paddingBottom: 'calc(4px + env(safe-area-inset-bottom, 0px))', background: 'linear-gradient(180deg, rgba(30,58,95,0.55) 0%, rgba(12,24,48,0.88) 100%)', backdropFilter: 'blur(40px) saturate(1.8)', WebkitBackdropFilter: 'blur(40px) saturate(1.8)', borderTop: '0.5px solid rgba(147,197,253,0.12)', boxShadow: 'inset 0 0.5px 0 rgba(147,197,253,0.15), 0 -2px 20px rgba(0,0,0,0.4)' }}>
        {newRound && (
          <button onClick={() => setTabyLivePillOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '6px 0 5px', borderBottom: '0.5px solid rgba(147,197,253,0.12)', background: 'rgba(212,175,55,0.12)', border: 'none', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: '#FFD56B' }}>●</span>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: '#FFD56B', letterSpacing: 0.8, fontWeight: 600 }}>RUNDA AKTIV · {roundScores.length}/18 HÅL · {totalStab}P</span>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'rgba(255,213,107,0.8)' }}>{tabyLivePillOpen ? '▼' : '▲'}</span>
          </button>
        )}
      ...
      {tabyView === 'leaderboard' && (
        <div style={{ padding: '0 16px' }}>
            ...
            return (
              <button onClick={() => (setTabyUser(prev => ({ ...prev, statsViewPid: leader.id })), setTabyView('stats'))}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 28, fontWeight: 700, color: '#D4A017', lineHeight: 1 }}>{leader.stats?.meritTotal ?? '—'}</div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 7, color: 'rgba(147,197,253,0.5)', letterSpacing: 1.5, marginTop: 3 }}>MERIT-POÄNG</div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'rgba(240,244,255,0.4)', marginTop: 4 }}>PI {leader.stats?.pi || 0} · {leader.stats?.fullRounds || 0}R</div>
                  </div>
                </div>
              </button>
            )
            ...
            <div style={{ display: 'flex', gap: 4, padding: '0 10px 8px', overflowX: 'auto', scrollbarWidth: 'none' }}>
              {[
                ...
                <button key={opt.k} onClick={() => setTabySortBy(opt.k)} style={{
                  ...
                  background: tabySortBy === opt.k ? `${opt.c}22` : 'rgba(147,197,253,0.04)',
                  border: `0.5px solid ${tabySortBy === opt.k ? opt.c : 'rgba(147,197,253,0.15)'}`,
                  color: tabySortBy === opt.k ? opt.c : 'rgba(240,244,255,0.5)',
                  fontWeight: tabySortBy === opt.k ? 700 : 400
                }}>{opt.l}</button>
              ))}
            </div>
            {playerStats.map((pl, idx) => {
              ...
              return (
                <div key={pl.id}
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0, display:'flex', alignItems:'center', gap:6 }} onClick={(e) => { e.stopPropagation(); setMeritBreakdownPid(pl.id) }}>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 18, fontWeight: 700, color: idx === 0 ? '#D4A017' : '#F0F4FF', lineHeight: 1 }}>{st.meritTotal ?? 0}</div>
          ...
          {meritBreakdownPid && (() => {
            const pl = playerStats.find(p => p.id === meritBreakdownPid)
            if (!pl) return null
            const s = pl.stats
            ...
            return (
              <div onClick={() => setMeritBreakdownPid(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(6,14,30,0.85)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, backdropFilter: 'blur(8px)' }}>
                <div onClick={e => e.stopPropagation()} style={{ background: 'linear-gradient(180deg, #14294A 0%, #0C1830 100%)', border: '1px solid rgba(212,160,23,0.3)', borderRadius: 20, padding: 20, maxWidth: 480, width: '100%', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
                  {/* Header */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
                    {pl.image_url ? <img src={pl.image_url} style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', border: '1.5px solid rgba(212,160,23,0.4)' }} /> : <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'rgba(147,197,253,0.1)', border: '1.5px solid rgba(212,160,23,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#93C5FD' }}>{pl.name?.charAt(0)}</div>}
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: 'var(--serif)', fontSize: 20, color: '#F0F4FF' }}>{pl.nickname}</div>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'rgba(147,197,253,0.5)', letterSpacing: 1.5, marginTop: 2 }}>MERIT-BREAKDOWN</div>
                    </div>
                    <button onClick={() => setMeritBreakdownPid(null)} style={{ background: 'rgba(147,197,253,0.08)', border: '0.5px solid rgba(147,197,253,0.2)', color: '#93C5FD', fontSize: 16, cursor: 'pointer', width: 32, height: 32, borderRadius: 10 }}>✕</button>
                  </div>
                  ...
                  {/* Bästa 8 rundor */}
                  {s.best8 && s.best8.length > 0 && (
                    <div style={{ marginBottom: 16, padding: 12, background: 'rgba(147,197,253,0.04)', border: '0.5px solid rgba(147,197,253,0.15)', borderRadius: 10 }}>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 8, color: 'rgba(147,197,253,0.5)', letterSpacing: 2, marginBottom: 8 }}>PI = SNITT AV DINA {s.best8.length} BÄSTA RUNDOR</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {s.best8.map((r, i) => (
                          <div key={i} style={{ padding: '4px 8px', background: 'rgba(147,197,253,0.08)', borderRadius: 6, fontFamily: 'var(--mono)', fontSize: 11, color: '#93C5FD', fontWeight: 600 }}>{r.total}p</div>
                        ))}
                      </div>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'rgba(240,244,255,0.4)', marginTop: 8 }}>Snitt: {s.pi}p</div>
                    </div>
                  )}

                  {/* Events */}
                  {s.eventResults && s.eventResults.length > 0 && (
                    <div style={{ marginBottom: 16, padding: 12, background: 'rgba(212,160,23,0.06)', border: '0.5px solid rgba(212,160,23,0.2)', borderRadius: 10 }}>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 8, color: 'rgba(212,160,23,0.7)', letterSpacing: 2, marginBottom: 8 }}>EVENT-PLACERINGAR</div>
                      {s.eventResults.map((e, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', fontSize: 12 }}>
                          <span style={{ color: '#F0F4FF' }}>{e.name}</span>
                          <span style={{ fontFamily: 'var(--mono)', color: '#D4A017' }}>#{e.position} · {e.points}p</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {/* H2H detaljer */}
                  {myH2H.length > 0 && (
                    <div style={{ padding: 12, background: 'rgba(74,222,128,0.06)', border: '0.5px solid rgba(74,222,128,0.2)', borderRadius: 10 }}>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 8, color: 'rgba(74,222,128,0.7)', letterSpacing: 2, marginBottom: 8 }}>H2H-MATCHER · {s.h2hWins}-{h2hDecided.length - s.h2hWins}{h2hDraw.length > 0 ? ` (+${h2hDraw.length} oavgjord)` : ''}</div>
                      {myH2H.map((m, i) => {
                        const opponent = tabyPlayers.find(p => p.id === (m.player1_id === pl.id ? m.player2_id : m.player1_id))
                        const isWin = m.winner_id === pl.id
                        const isDraw = m.winner_id === null
                        return (
                          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', fontSize: 11 }}>
                            <span style={{ color: '#F0F4FF' }}>vs {opponent?.nickname || '?'}</span>
                            <span style={{ fontFamily: 'var(--mono)', color: isWin ? '#4ADE80' : isDraw ? 'rgba(147,197,253,0.6)' : 'rgba(232,99,74,0.8)' }}>
                              {isWin ? '🏆 vinst' : isDraw ? '🤝 oavgjord' : 'förlust'}
                            </span>
                          </div>
                        )
                      })}
...
function DIOApp({ onSwitchMode }) {
  ...
  const [crsApiQuery, setCrsApiQuery] = useState('')
  const [crsApiResults, setCrsApiResults] = useState([])
        ...
        <div style={{ marginTop: 16, borderTop: `1px solid ${bd}`, paddingTop: 12 }}>
          ...
          {showCourseForm && (
            <div style={{ marginTop: 10, display:'flex', flexDirection:'column', gap:8 }}>
              ...
              <div style={{ fontSize:10, color:'var(--cream-muted)', textAlign:'center' }}>— eller fyll i manuellt —</div>
...
Max 2-3 meningar. Svenska. Använd spelarens nickname.`
  ...
  const roastPlayer = async (targetPlayer) => {
    ...
    try {
      ...
      // Öppna full-screen roast-modal med score-context
      const scoreContext = stats.holesPlayed > 0
  ...
  // ===== APP =====
  return (
    <PullToRefresh color="#D4AF37" bg="rgba(0,0,0,0.7)" onRefresh={async () => {
      await Promise.all([
    ...
    <div>
      ...
      {guideHole && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 500, display: 'flex', flexDirection: 'column', overflowY: 'auto' }} onClick={() => setGuideHole(null)}>
          ...
          {/* Banguide-bild från LiveCaddie CloudFront */}
          {(() => {
            const courseName = course?.name?.includes('Skog') ? 'Skogsbanan' : 'Parkbanan'
            const imgUrl = holeImages?.[courseName]?.[guideHole]
            if (!imgUrl) return null
            return (
              <div onClick={e => e.stopPropagation()}>
                <img src={imgUrl} alt={`Hål ${guideHole}`} style={{ width: '100%', maxHeight: '45vh', objectFit: 'cover', display: 'block' }} />
                <div style={{ fontSize: 9, color: 'var(--cream-muted)', textAlign: 'center', padding: '3px 0', background: 'rgba(0,0,0,0.5)' }}>LiveCaddie · Hooks GK</div>
              </div>
            )
          })()}
          {/* Flyover video */}
      ...
      <div className="status-bar" style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
        <div className="island-pill" style={{
          ...
          {(() => {
            ...
            return (
              <button onClick={() => setLivePillOpen(o => !o)} style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', background: isComplete ? 'rgba(74,222,128,0.28)' : 'rgba(212,175,55,0.32)', borderRadius: 12, border: isComplete ? '1px solid rgba(74,222,128,0.6)' : '1px solid rgba(212,175,55,0.55)', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}>
                <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: '#FFD56B', letterSpacing: 0.5, fontWeight: 700 }}>R{selRound}</span>
                <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: '#FAF8F0', fontWeight: 500 }}>{playedCount}/18</span>
                <span style={{ fontSize: 12, fontFamily: 'var(--mono)', fontWeight: 700, color: isComplete ? '#86EFAC' : '#FFD56B' }}>{myTotal}p</span>
                <span style={{ fontSize: 10, color: 'rgba(250,248,240,0.85)', fontFamily: 'var(--mono)', fontWeight: 500 }}>{courseShort}</span>
                <span style={{ fontSize: 10, color: 'rgba(250,248,240,0.7)' }}>{livePillOpen ? '▲' : '▼'}</span>
              </button>
            )
          })()}
          {(() => {
      ...
      {roastModal && (
        <div onClick={() => setRoastModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, animation: 'roastFadeIn 0.25s ease-out' }}>
          <style>{`
            @keyframes roastFadeIn { from { opacity: 0; transform: scale(0.92) } to { opacity: 1; transform: scale(1) } }
            @keyframes roastFlame { 0%, 100% { transform: scale(1) rotate(-2deg); filter: drop-shadow(0 0 12px rgba(232,99,74,0.6)) } 50% { transform: scale(1.08) rotate(2deg); filter: drop-shadow(0 0 20px rgba(232,99,74,0.9)) } }
          `}</style>
          <div onClick={e => e.stopPropagation()} style={{ background: 'linear-gradient(135deg, rgba(232,99,74,0.18), #0B1410)', border: '1.5px solid rgba(232,99,74,0.5)', borderRadius: 20, padding: 24, maxWidth: 380, width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(232,99,74,0.4)' }}>
            {/* Header — From → Target */}
            <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: '#FF8A6B', letterSpacing: 2, marginBottom: 14, fontWeight: 700 }}>🔥 CADDIE ROAST</div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 4 }}>
              <Av p={roastModal.from} size={32} />
              <span style={{ fontSize: 14, color: '#FAF8F0', fontFamily: 'var(--mono)' }}>{roastModal.from?.nickname}</span>
              <span style={{ fontSize: 16, color: '#FF8A6B' }}>→</span>
              <Av p={roastModal.target} size={40} />
              <span style={{ fontSize: 16, fontWeight: 700, color: '#FAF8F0' }}>{roastModal.target.nickname}</span>
            </div>
            {/* Score-kontext under target */}
            <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'rgba(250,248,240,0.7)', letterSpacing: 0.5, marginBottom: 20 }}>{roastModal.scoreContext}</div>
            {/* Flam-divider */}
            <div style={{ fontSize: 28, animation: 'roastFlame 2s ease-in-out infinite', marginBottom: 14 }}>🔥</div>
            {/* Själva roasten */}
            <div style={{ fontSize: 18, lineHeight: 1.55, color: '#FFFFFF', fontFamily: 'var(--serif)', fontStyle: 'italic', padding: '0 8px', marginBottom: 22, fontWeight: 500, textShadow: '0 1px 2px rgba(0,0,0,0.5)' }}>
              "{roastModal.text}"
            </div>
            {/* Källa */}
            <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'rgba(250,248,240,0.45)', letterSpacing: 1, marginBottom: 16 }}>— GENERATED BY CADDIE AI · POSTAT I CHAT · PUSH SKICKAD</div>
            {/* Actions */}
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => { roastPlayer(roastModal.target); setRoastModal(null) }} disabled={roastingPid === roastModal.target.id} style={{ flex: 1, padding: '12px', background: 'rgba(232,99,74,0.2)', border: '1px solid rgba(232,99,74,0.5)', borderRadius: 10, color: '#FF8A6B', fontSize: 13, fontWeight: 600, cursor: roastingPid ? 'wait' : 'pointer', fontFamily: 'var(--mono)' }}>🔁 Ny roast</button>
              <button onClick={() => setRoastModal(null)} style={{ flex: 1, padding: '12px', background: '#FFD56B', border: 'none', borderRadius: 10, color: '#0A0A08', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>OK</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="page-content" onClick={() => showNotifs && setShowNotifs(false)}>
        ...
        {view === 'info' && (<>
            ...
            <div style={{ padding: '12px 14px', borderBottom: '0.5px solid var(--card-border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--cream-muted)' }}>{prizeConfig.perPlayer} kr/person · 3 kategorier</div>
                </div>
              </div>
              <div style={{ fontSize: 11, color: 'var(--cream-dim)', lineHeight: 1.6, marginBottom: 12, fontStyle: 'italic' }}>
                "Vi hade tänkt köra lönebaserad prislista, men då måste Mange ge en Ferrari medan Matthis kommer undan med fickludd… så kan inte ha det… vi är ju ändå inte kommunister (fast man vet ju inte riktigt med Plus One 😏)"
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, marginBottom: 10 }}>
                <div style={{ background: 'rgba(212,175,55,0.1)', borderRadius: 8, padding: '8px', border: '0.5px solid rgba(212,175,55,0.25)', textAlign: 'center' }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 7, color: 'var(--gold)', letterSpacing: 1, marginBottom: 4 }}>INDIVIDUELLT</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--gold)', lineHeight: 1 }}>6</div>
                  <div style={{ fontSize: 9, color: 'var(--cream-muted)', marginTop: 4 }}>Rank 1–6</div>
                </div>
                <div style={{ background: 'rgba(74,222,128,0.08)', borderRadius: 8, padding: '8px', border: '0.5px solid rgba(74,222,128,0.2)', textAlign: 'center' }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 7, color: '#4ADE80', letterSpacing: 1, marginBottom: 4 }}>LAG</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#4ADE80', lineHeight: 1 }}>3</div>
                  <div style={{ fontSize: 9, color: 'var(--cream-muted)', marginTop: 4 }}>Vinnande lag</div>
                </div>
                <div style={{ background: 'rgba(232,99,74,0.08)', borderRadius: 8, padding: '8px', border: '0.5px solid rgba(232,99,74,0.2)', textAlign: 'center' }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 7, color: '#E8634A', letterSpacing: 1, marginBottom: 4 }}>TRÖST</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#E8634A', lineHeight: 1 }}>3</div>
                  <div style={{ fontSize: 9, color: 'var(--cream-muted)', marginTop: 4 }}>Förtjänat det</div>
                </div>
              </div>
              <div style={{ fontSize: 11, color: 'var(--cream-dim)', lineHeight: 1.6 }}>
                <strong style={{ color: 'var(--cream)' }}>Individuellt (6 priser):</strong> 1:an plockar först, sen ner i ranking till 6:an.<br/>
                <strong style={{ color: '#4ADE80' }}>Lag (3 priser):</strong> Vinnande lagets spelare delar 3 priser.<br/>
                <strong style={{ color: '#E8634A' }}>Tröstpriser (3):</strong> För de som "förtjänat det" — daily losers, mest blowups, värsta swing, etc. Bestäms på söndagen.
              </div>
              <div style={{ fontSize: 10, color: 'var(--cream-muted)', marginTop: 8, fontStyle: 'italic' }}>Förslag: golf-prylar, whisky, kläder, gadgets. Matthis: ingen begagnad träklubba från 1987.</div>
            </div>
            {/* Sidobets */}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 14, padding: 14, marginBottom: 14, border: '0.5px solid var(--card-border)' }}>
            {[
              ...
              { kat: 'Prisbordet ⭐', items: ['2× priser per person till bordet (12 totalt)','Budget ~500 kr/person','6 individuella · 3 till lag-vinnare · 3 tröstpriser','Förslag: golf, whisky, kläder, gadgets — fritt fram'] },
        ...
        {view === 'settings' && isAdmin && (<>
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
              {[
                ...
                { t: '⭐ Glöm inte 2 prisbordet', b: 'Påminnelse: 2 priser per person, ~500 kr budget. 12 priser totalt fördelat på individuellt (6), lag (3) och tröstpriser (3).' },
              ].map((q, i) => (
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 14, marginBottom: 14 }}>
            ...
            <div style={{ background: 'rgba(212,175,55,0.05)', borderRadius: 10, padding: 12, border: '0.5px solid rgba(212,175,55,0.15)', fontSize: 11, color: 'var(--cream-dim)', lineHeight: 1.6 }}>
              <strong style={{ color: 'var(--gold)' }}>📋 12 priser fördelas så här:</strong><br/>
              · 6 individuella (rank 1–6)<br/>
              · 3 till vinnande laget<br/>
              · 3 tröstpriser (förtjänade)
            </div>
          </div>
