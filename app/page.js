    ...
    <div>
      ...
      {roastModal && (
        <div onClick={() => setRoastModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, animation: 'roastFadeIn 0.25s ease-out' }}>
          ...
          `}</style>
          <div onClick={e => e.stopPropagation()} style={{ background: 'linear-gradient(135deg, rgba(232,99,74,0.12), rgba(11,20,16,0.95))', border: '1px solid rgba(232,99,74,0.4)', borderRadius: 20, padding: 24, maxWidth: 380, width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(232,99,74,0.3)' }}>
            {/* Header — From → Target */}
            <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'rgba(232,99,74,0.7)', letterSpacing: 2, marginBottom: 14 }}>🔥 CADDIE ROAST</div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 4 }}>
              <Av p={roastModal.from} size={32} />
              <span style={{ fontSize: 14, color: 'var(--cream-dim)', fontFamily: 'var(--mono)' }}>{roastModal.from?.nickname}</span>
              <span style={{ fontSize: 16, color: 'var(--coral)' }}>→</span>
              <Av p={roastModal.target} size={40} />
              <span style={{ fontSize: 16, fontWeight: 600, color: 'var(--cream)' }}>{roastModal.target.nickname}</span>
            </div>
            {/* Score-kontext under target */}
            <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'rgba(250,248,240,0.45)', letterSpacing: 0.5, marginBottom: 20 }}>{roastModal.scoreContext}</div>
            {/* Flam-divider */}
            <div style={{ fontSize: 24, animation: 'roastFlame 2s ease-in-out infinite', marginBottom: 12 }}>🔥</div>
            {/* Själva roasten */}
            <div style={{ fontSize: 17, lineHeight: 1.5, color: 'var(--cream)', fontFamily: 'var(--serif)', fontStyle: 'italic', padding: '0 8px', marginBottom: 20 }}>
              "{roastModal.text}"
            </div>
            {/* Källa */}
            <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'rgba(250,248,240,0.3)', letterSpacing: 1, marginBottom: 16 }}>— GENERATED BY CADDIE AI · POSTAT I CHAT · PUSH SKICKAD</div>
            {/* Actions */}
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => { roastPlayer(roastModal.target); setRoastModal(null) }} disabled={roastingPid === roastModal.target.id} style={{ flex: 1, padding: '12px', background: 'rgba(232,99,74,0.15)', border: '1px solid rgba(232,99,74,0.4)', borderRadius: 10, color: 'var(--coral)', fontSize: 13, fontWeight: 500, cursor: roastingPid ? 'wait' : 'pointer', fontFamily: 'var(--mono)' }}>🔁 Ny roast</button>
              <button onClick={() => setRoastModal(null)} style={{ flex: 1, padding: '12px', background: 'var(--gold)', border: 'none', borderRadius: 10, color: '#0A0A08', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>OK</button>
            </div>
          </div>
        </div>
      )}
