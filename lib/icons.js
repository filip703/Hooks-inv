...
// LakeBadge = Täby's equivalent — midnight blue + gold ring
export function LakeBadge({ children, size = 28, active = false }) {
  return (
    <div style={{
      width: size, height: size, borderRadius: '50%',
      background: active ? '#D4A017' : '#1E3A5F',
      border: `1.5px solid ${active ? '#1E3A5F' : '#D4A017'}`,
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      flexShrink: 0,
      transition: 'all 0.3s ease',
    }}>
...
// 🔥 SMOKE SIGNAL — för Slasken (Chat döpt till Slask)
// Tre rökpuffar som stiger upp + en liten flame längst ner
export function IconSmokeSignal({ size = 18, color = 'currentColor' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
      {/* Flame/eld längst ner */}
      <path d="M10 22 Q12 20 14 22" />
      <path d="M11 20 Q12 18.5 13 20" fill={color} stroke="none" />
      {/* Rökpuff 1 — minst, längst upp */}
      <path d="M14 5 Q15.5 3.5 17 5 Q18 6 16.5 7 Q15 7.5 14 6.5 Z" />
      {/* Rökpuff 2 — mellan */}
      <path d="M9 9 Q11 7 13 9 Q14 10.5 12 11.5 Q10 12 9 10.5 Z" />
      {/* Rökpuff 3 — störst, närmast elden */}
      <path d="M7 14 Q9 11.5 12 13 Q14.5 14.5 12.5 16.5 Q9.5 17.5 7 15.5 Z" />
    </svg>
  )
}
