...
export const specialHoles = {
  ...
  3: { ld: 10, np: 3, doubleStart: 16 },    // R3 Park (flyttad från Skog): LD hål 10 (498m monster), NP hål 3
...
export const courses = {
  ...
  Parkbanan: {
    ...
    holes: [
      ...
      { hole: 6, par: 3, hcp: 13, meters: 119, tip: '119m – KORTASTE HÅLET PÅ HOOKS. Sand wedge eller gap wedge, max. Green skyddad av bunker höger. Spela mot vänster halva. Fredrik tar full swing 9-järn för säkerhets skull. Alla: *suck*.', notice: 'Hål 6+7 sammanslås till nytt korthål 2026. Följ skyltning. Boll i avgränsat område → droppa i anvisad droppruta.' },
      { hole: 7, par: 3, hcp: 17, meters: 141, tip: '141m – tredje par 3 på fyra hål. PW eller 9-järn. Inget vatten, inga träd, bara green och bunker. Parkbanans "andningshål". Magnus har tappat tre bollar hittills. Det är först hål 7.', notice: 'Hål 6+7 sammanslås till nytt korthål 2026. Följ skyltning. Boll i avgränsat område → droppa i anvisad droppruta.' },
...
export function checkStreaks(holeScores) {
  // holeScores = [{hole, pts, strokes, par}, ...] sorted by hole
  // Hot Hand = 2 RIKTIGA birdies i rad (gross strokes < par), inte netto-stableford.
  // Cold Turkey = 2 nollor i rad (0 stableford-poäng).
  const result = { hotHand: 0, coldTurkey: 0 }
  let birdieStreak = 0, zeroStreak = 0
  for (const s of holeScores) {
    const isBirdie = s.strokes > 0 && s.par > 0 && s.strokes < s.par // gross birdie
    if (isBirdie) { birdieStreak++; zeroStreak = 0 }
    else if (s.pts === 0) { zeroStreak++; birdieStreak = 0 }
    else { birdieStreak = 0; zeroStreak = 0 }
    if (birdieStreak >= 2) result.hotHand++
    if (zeroStreak >= 2) result.coldTurkey++
  }
...
// Hål-bilder från LiveCaddie / CloudFront (mappade via scorecard-scraping 2026-05-01)
// null = ingen bild → banguide-vyn visar flyover-video som fallback
const SB_HOLES = 'https://swagnjpgddfakncovglo.supabase.co/storage/v1/object/public/inv-images/hooks'
// Banguide-bilder: lokala paths (public/hooks/) — Playwright screenshots från LiveCaddie
const LOCAL = '/hooks'
export const holeImages = {
  Parkbanan: Object.fromEntries(Array.from({length: 18}, (_, i) => [i+1, `${LOCAL}/parkbanan/hole-${i+1}.png`])),
  Skogsbanan: Object.fromEntries(Array.from({length: 18}, (_, i) => [i+1, `${LOCAL}/skogsbanan/hole-${i+1}.png`])),
}
...
export const playerRoasts = {
  ...
  matthis: [
    'Försvarande mästare. Han berättar gärna om det. Länge. Vid varje hål.',
    'Hela vintern på matta i ett garage i Stockholm. Slag mot en vägg som aldrig slagit tillbaka. Det hjälpte knappt.',
    'Tekniken är jobbig för honom — inte golf-tekniken, utan tekniken generellt. Appen, GPSen, en knapp. Vadå Bluetooth?',
    'Bollen i sjön på hål 1. Bollen i sjön på hål 3. Bollen i sjön på hål 9. Han kallar det "aggressivt spel".',
    'Har memorerat OB-reglerna i minst fyra europeiska länder. Jobbar flitigt på att behöva dem.',
    'Gnäller redan på parkeringen. Gnäller på tee. Gnäller in. Gnäller ut. Den ende som är mer konsekvent är Marcus med sina öl.',
    'Vann 2025. Har aldrig pratat om något annat sedan dess. Inte en enda gång.',
    'Ser dåligt, gnäller friskt. Känslan av att han knappt ser bollen men ändå har världens starkaste åsikt om swingen.',
    'Tränade hela vintern. R1 hål 1 — bollen i sjön. Fyra månader summerade på 40 meter.',
    ...
    'Om gnäll gav stableford-poäng hade Matthis vunnit 2026 också. Tyvärr gör det inte det.',
    'Utan Matthis ingen DIO. Och utan DIO ingen Matthis att häckla. Cirkelns helighet.',
    'Ambulerande bartender numera. Dinkar drinkar mellan slagen, levererar till bollen, och tackar sig själv för servicen. "Tackar!"',
    'Kass på Shack. Lika kass på Shack som på golf. Den enda konsekvensen i hans liv.',
    'Springer omkring som en bar på två ben. Drinkar till alla, slag till ingen. "Tackar!" säger han, till sig själv.',
  ],
  marcus: [
    ...
    'Har med sig en fårpung på banan som vi dricker shots ur. Ingen vet varför. Ingen vågar fråga. Alla dricker ändå.',
    'Mitt i rundan, på hål 7, undrar Marcus högt vad alla egentligen tycker om att slicka anus. Ingen vet hur vi hamnade där. Marcus vet alltid hur vi hamnade där.',
    'Fårpungen kommer fram vid första svackan. Och Marcus svackar gärna, för fårpungens skull.',
    'Jensade sig själv rejält igår — full som en kille från Blekinge som kör för hårt. Det är nu ett verb. Tack Marcus.',
  ],
  fredrik: [
    ...
    'Mr Piano Man. Sätter sig vid flygeln på kvällen och plötsligt har Iceman känslor. På banan? Fortfarande absolut noll.',
    'Spelar piano bättre än han spelar ur bunkern. Vilket inte säger mycket — men det är ändå det finaste vi kan säga.',
  ],
  magnus: [
    ...
    'Har en FatBoy. Eller ÄR en FatBoy. Juryn är fortfarande ute, och kommer nog förbli det.',
    'Konstant öppen telefonlinje till Johanna som vill veta allt. Varje slag rapporteras live. Magnus eget kommentatorsbås, med en publik på exakt en person.',
    'Spelar med Johanna i örat hela rundan. Hon kan hans score bättre än han själv — och har starkare åsikter om hybriden.',
    'As rik, men telefonlinjen till Johanna är den dyraste investeringen. Den betalar aldrig tillbaka, den bara vill veta allt.',
  ],
  martin: [
    ...
    'Älskar farfars intervjuer mer än han älskar golf. Och han spelar varenda runda, så det säger en del om intervjuerna.',
    'KUNG på Tre i rad. Inte golf alltså — spelet. Det enda Martin faktiskt vinner under hela helgen.',
    'Obesegrad i Tre i rad. HCP 40 på banan, HCP 0 vid spelbordet. Helt fel spel att vara grym på.',
    'Sätter på en gammal farfars-intervju på väg till banan för att komma i rätt mentalt läge. Det syns inte på scoren.',
  ],
}
...
// ===== AI Roast Profiler =====
// Strukturerad roast-data per spelare som matas in i Caddie API.
// Innehåller karaktärsdrag, intern jargong och "sore spots" — AI:n
// kombinerar detta med live-prestanda för att generera personliga roasts.
export const roastProfiles = {
  filip: {
    persona: 'Make Golf-CEO som tycker han är pro men slår OB hela tiden. Vann 2022+2023, har redan glömt hur det känns.',
    soreSpots: [
      'Long Gone-nick: pucken landar i skogen, fairway:n förblir orörd',
      'CEO på Make Golf men ser ut som en lost weekend-warrior på rangen',
      'förlorade 2025-finalen till en arbetslös (Matthis) — det stings än idag',
      'pratar om Make Golf på range trots att ingen frågat',
      'HCP 8.6 men spelar som 18.5 när det räknas',
      'snygg PWA men trasiga slag — bygger appar för andra som golfar bra',
      'pitchar Make Golf till caddien som inte fattar svenska',
      'sa "i år tar jag det igen" i januari, har inte vunnit en runda sen',
      'försökte byta klubba mitt i swing pga "ny känsla"',
      'Mr Vain-nick: speglar är bästa wing-man',
      'jobbar i hospitality tech men kan inte boka egen tee-tid',
      'vann 2 år i rad men det var 2022-2023 (en miljon år sen)',
    ],
    angles: ['Make Golf-pitcher mid-round', 'OB-slag på "lätta" hål', 'Mr Vain-spegelgrejer', '2025-finalförlusten', 'hospitality-tech-jargong på range', 'app-bug istället för golf'],
    forbidden: ['hans dotter', 'familj', 'fru'],
    tone: 'lite avundsjuk: ni spelar med er egen CEO men han suger',
  },
  matthis: {
    persona: 'Pro Am-wannabe (THE GRINDER) som är arbetslös, har all tid att grinda men ingen riktig grind. Vann 2025 på en lucky shot.',
    soreSpots: [
      'ARBETSLÖS — har all tid i världen, ändå dålig',
      'Pro Am-nick: amatör som tror han är pro',
      '2025-vinsten var en lucky shot på hål 17, inte talang',
      'jobbsöker fortfarande, runda 100 ansökningar deep',
      'kommer på rangen 10:00 på en tisdag (vad fan ska han annars göra)',
      'pratar om "konsultuppdrag" men menar Tinder-dates',
      'försökte säga "jag är mellan jobb just nu" hela 2024',
      'fakar Spotify Premium för att inte betala 99 kr',
      'Magnus köper hans öl, han säger "tack bror"',
      'LinkedIn-profil säger "open to work" sen Q3 2024',
      'fick credit för försenad hyra senaste 3 månaderna',
      'sov över hos Marcus två gånger förra månaden',
      'spelar fyra rundor i veckan men 0 jobbintervjuer',
      'ambulerande bartender på banan — dinkar drinkar mellan slag och tackar sig själv ("Tackar!")',
      'kass på Shack — lika kass som på golf, men mer konsekvent',
      'levererar drinkar till alla i bollen men kan inte leverera ett rakt slag',
    ],
    angles: ['arbetslöshets-skämt', 'mellan jobb-eufemismer', 'LinkedIn open-to-work', 'lucky 2025-vinst', 'Magnus betalar', 'Pro Am-amatörism', 'tisdag 10:00 på range', 'sov över hos andra', 'ambulerande bartender Tackar', 'kass på Shack'],
    forbidden: [],
    tone: 'brutal — han ska få mest skit. ARBETSLÖS-jokes räcker långt',
  },
  martin: {
    persona: 'Plus One-rookie HCP 40 med kommunistiska tendenser (intern joke). Plays maybe twice a year.',
    soreSpots: [
      'HCP 40 (rookie cap, kan inte gå högre — det är taket)',
      'kommunistiska tendenser: hatar kapitalism men älskar Hooks Spa (510 kr/natt)',
      'spelade exakt 2 rundor 2019, sen ingen golf förrän nu',
      'Plus One-nick: alltid ett slag bakom alla, varje hål',
      'kanske egentligen är dansk (vem vet)',
      'pratar om "klassklamp" mellan slag på par 5',
      'läser Marx på driving range mellan slag',
      'sa "äganderätten över greens borde avskaffas"',
      'vägrar tipsa caddien "av principskäl"',
      'tar 14 slag på hål 1, säger "men jag har inte spelat på 7 år"',
      'kommer i armékläder till klubbhuset',
      'driver röd elbil men kallar Tesla "kapitalist-leksak"',
      'försöker organisera spelarna i en "fackförening"',
      'älskar farfars gamla intervjuer mer än golf — sätter på dem på vägen till banan',
      'KUNG på Tre i rad (spelet, inte golf) — obesegrad vid spelbordet, hopplös på banan',
    ],
    angles: ['HCP 40 är taket', 'kommunist möter lyxspa', 'klassklamp på fairway', 'Marx på range', '7 års paus', 'Plus One bakom alla', 'fackförening för golfare', 'röd Tesla-paradox', 'farfars intervjuer', 'kung på Tre i rad (spelet)'],
    forbidden: [],
    tone: 'mjuk men ironisk — han är rookien, ge honom credit för att han ens dök upp',
  },
  marcus: {
    persona: 'Five o\'Clock — analytisk spreadsheet-nörd som kan stableford-tabellen utantill men 3-puttar greener. Vann 2021 (gammalt nyhet).',
    soreSpots: [
      'Five o\'Clock-nick (det är klockslaget, inte exemplary)',
      'vann 2021 men ingen pratar om det längre — det var en pandemi-vinst',
      'har Excel för sina egna stats, drog ner Apple Numbers-aktien',
      'kan stableford-tabellen utantill men 3-puttar varje rond',
      'tar 12 minuter att välja klubba på en par 3',
      'frågar caddien om vindriktningen i grader (45° NV?)',
      'läser greenens lutning som om det vore en heat-map',
      'har en spreadsheet med h2h-stats mot varenda spelare sen 2018',
      'mätte regndroppar med scorecardet under rond 3',
      'försöker overlay GPS-data på en pappers-yardage-bok',
      'sa "jag har en modell som visar att jag borde vunnit 2023"',
      'har lagt 4 timmar i pre-game analys, 5 timmar på rangen, 0 timmar på short game',
      'har med sig en fårpung på banan som alla dricker shots ur — ingen vet varför, ingen vågar fråga',
      'undrar mitt i rundan, på hål 7, vad alla tycker om att slicka anus — ingen vet hur vi hamnade där',
    ],
    angles: ['Excel-överanalys', '3-putt-stats', 'klubbval-paralys', '12 minuters tankegång', '2021-vinsten är gammal', 'h2h-spreadsheet', 'modeller säger han borde vinna', 'analytiker som glömmer kärnan', 'fårpungs-shots', 'opassande frågor mitt i rundan'],
    forbidden: [],
    tone: 'analytisk roast — räkna hans 3-putts statistiskt, citera "modeller"',
  },
  fredrik: {
    persona: 'The Fossil — gammal old fashioned-drinkare som glömmer sin score från förra hålet. Veteran utan vinster.',
    soreSpots: [
      'The Fossil-nick (verkligen fossil — daterar tillbaka till blyhängarna)',
      'HCP 22.3 men säger "jag är runt 15" till alla',
      'glömmer score från förra hålet, frågar två gånger',
      'beställer Old Fashioned innan hål 1, second one på hål 6',
      'sista att lämna 19th hole, alltid',
      'pratar om "när jag spelade Royal Birkdale -86" en gång per rond',
      'använder en putter från 1992 "den har historia"',
      'pratar med greenkeepers som om de var jämngamla',
      'tycker progressiv golf "förstör spelet"',
      'har glasögon på pannan, frågar vart de tog vägen',
      'sa "fairwaysen är inte vad de var" — de är exakt samma',
      'vägrar GPS-klocka, "min penna och papper räcker"',
      'står 30 sekunder med vinden i ansiktet "för att läsa banan"',
      'har aldrig vunnit DIO men har varit med på "alla år"',
      'Mr Piano Man — sätter sig vid flygeln på kvällen och får plötsligt känslor han aldrig visar på banan',
    ],
    angles: ['Old Fashioned-räkning', 'minnesförlust på fairway', 'Royal Birkdale -86', 'fossil-teknologi', 'glasögon på pannan', 'progressiv golf "förstör spelet"', 'aldrig vinst men alltid med', '19th hole-mästare', 'Mr Piano Man'],
    forbidden: [],
    tone: 'whisky-soaked roast — referera drycker, gamla år, papper-och-penna',
  },
  magnus: {
    persona: 'The Hybrid — AS RIK men snålaste, slår bara hybrid-slag, köper aldrig en runda. Vann 2024 på en svag turnering.',
    soreSpots: [
      'AS RIK men SNÅLAST — köper aldrig en runda',
      'The Hybrid-nick (slår bara hybrid, vågar inte driver)',
      'Plan B-tendens (Plan A misslyckades nån gång 2019)',
      'vann 2024 men det var en svag turnering — Matthis var inte ens där',
      'har 7-siffrigt på kontot men säger "kanske vi splittar?"',
      'kör en gammal Volvo "för miljön" men har en Porsche i garaget',
      'köpte en hel set golfklubbor för 4000 kr på Tradera',
      'tar med egna toast-mackor till klubbhuset',
      'frågar om Hooks har "happy hour" på sin spa',
      'försökte förhandla på greenfee inför rond 3',
      'lägger Bingolotto på bordet inför avgörande hål — "jag har faktiskt 3 vinstrader"',
      'pratar om sina aktier men låter Matthis betala kaffet',
      'försökte få Filip att Swisha tillbaka 17 kr "från förra året"',
      'har en spreadsheet på vem som skuldar honom 50 kr',
      'har en FatBoy — eller ÄR en FatBoy, juryn är fortfarande ute',
      'konstant öppen telefonlinje till Johanna som vill veta allt — varje slag rapporteras live',
    ],
    angles: ['snålhet vs rikedom', 'happy hour på spa', 'gamla Volvo-skenet', 'Tradera-fynd', 'Bingolotto-vinstrader', 'Swish 17 kr-skuld', 'Plan B-existens', '2024-vinsten var svag', 'matthis betalar för Magnus', 'FatBoy', 'Johanna-telefonlinjen som vill veta allt'],
    forbidden: [],
    tone: 'pengar-jokes — han har råd, han bara vägrar. Snålheten är legendarisk',
  },
  rami: {
    persona: 'Säpo — låg profil, ingen vet vad han egentligen jobbar med, dök upp 2026 från ingenstans',
    soreSpots: [
      'Säpo-nick (extremt lågprofil, ingen vet något)',
      'HCP 23.7 (försöker bli bättre, lär ta tid)',
      'är ny i gänget, har inte intjänat respekt än',
      'svarar aldrig direkt på vad han jobbar med',
      'sa "konsult" tre gånger när någon frågade om jobbet',
      'har en svart bil utan registreringsskylt',
      'tar inte bilder, "förstår säkert varför"',
      'sa "den informationen är klassificerad" som svar på "vad är din HCP-utveckling"',
      'kollar tillbaka över axeln innan han slår',
      'försvann mellan hål 9 och 10 "för ett samtal"',
      'använder kontant på 19th hole',
    ],
    angles: ['mystisk jobbsituation', 'svart bil utan skyltar', 'klassificerad info', 'kollar över axeln', 'kontant betalning', 'försvinner mellan hål', 'konsult x3'],
    forbidden: [],
    tone: 'mystisk roast — spinna på att han är agent eller spion, paranoid undertext',
  },
}

// Builds a Caddie API prompt for roasting a specific player.
// Roterar angle + 2-3 random sore spots så roasterna varieras mellan körningar.
export function buildRoastPrompt(targetPlayer, recentStats, fromPlayer) {
  const profile = roastProfiles[targetPlayer.key]
  if (!profile) return null

  // Slumpa fram en angle + 2-3 sore spots så vi inte får samma roast varje gång
  const pickRandom = (arr, n) => {
    const shuffled = [...arr].sort(() => Math.random() - 0.5)
    return shuffled.slice(0, Math.min(n, arr.length))
  }
  const angle = profile.angles && profile.angles.length > 0 ? pickRandom(profile.angles, 1)[0] : null
  const spots = pickRandom(profile.soreSpots, 3)

  const statsLine = recentStats ? `Senaste: ${recentStats.lastHole ? 'hål ' + recentStats.lastHole + ' (' + recentStats.lastStrokes + ' slag, ' + recentStats.lastPts + 'p)' : 'inga slag än'} · Total ${recentStats.totalPts}p (${recentStats.holesPlayed}/18 hål)` : 'Ingen data än'

  return `Du är en brutal men kul vän i DIO-gänget (Douche Invitational Only — Sveriges mest dysfunktionella golf-vänner). Du roastar ${targetPlayer.name} ("${targetPlayer.nickname}", HCP ${targetPlayer.hcp}) på beställning av ${fromPlayer?.nickname || 'någon i gänget'}.

OFFRETS PERSONLIGHET: ${profile.persona}

ANGLE för denna roast (kör hårt på denna vinkel): ${angle || 'fri vinkel'}

SORE SPOTS du kan dra från (välj 1-2 max — DJUPT, inte ytligt):
${spots.map(s => `- ${s}`).join('\n')}

TON: ${profile.tone}
${profile.forbidden.length ? 'TABU (rör aldrig): ' + profile.forbidden.join(', ') : ''}

LIVE-DATA: ${statsLine}

REGLER:
- Skriv EN roast på svenska, 1-2 meningar max
- Använd nicknamen ("${targetPlayer.nickname}") MINST en gång
- Var specifik — referera EN sak (sore spot eller live-data), inte allt på en gång
- VARIERA stil per körning: ibland direkt jab, ibland sarkastisk observation, ibland fake-empati ("det är okej att…")
- INGEN positiv slutkläm, ingen "men du är ändå grym"
- Brutal humor, inte elak. Internt skämt-vibes — vi älskar varandra men hånar konstant.
- Inga emojis, ingen "haha" eller "lol"
- Undvik samma öppningsfras som tidigare roasts (variera "Wow,", "Imponerande,", "Klassiskt,")

Generera bara roasten, inget mer.`
}
...
export const achievements = [
  ...
  { id: 'hot_hand', icon: '🔥', title: 'Hot Hand', desc: '2 birdies i rad', check: (scores) => { let streak = 0; for (const s of scores) { if (s.stableford_points >= 3) { streak++; if (streak >= 2) return true } else streak = 0 } return false } },
